...
void RaftLog::AppendEntry(int64_t term,const Command& command)
{
    LogEntry entry(LastIndex()+1,term,command);
    entries_.emplace_back(entry);
}

bool RaftLog::AppendFrom(int64_t startIndex,const std::vector<LogEntry>& entries)
{
...
std::optional<LogEntry> RaftLog::GetEntry(int64_t index) const
{
    if(index<=0 || index>Size()) return std::nullopt;

    return entries_[index-1];           //!隐式构造optional 避免拷贝
}

std::vector<LogEntry> RaftLog::GetEntries(int64_t startIndex,int64_t endIndex) const
{
